...
Example:
  ...
  $0 -a \$subjid-t1.nii.gz -e IITSkull.nii.gz 

Required arguments:

     -d:  Image dimension                       2 or 3 (default) (for 2- or 3-dimensional image)
...
Optional arguments:
     ...
     -k:  keep temporary files?                 Keep or delete brain extraction/segmentation warps, etc (1/0).
     -q:  use floating point precision?         Use or don't use antsRegistration with floating point precision (1/0).
...
USE_FLOAT_PRECISION=1

if [[ $# -lt 3 ]] ; then
  Usage >&2
  exit 1
else
  while getopts "a:c:d:e:f:g:h:k:m:o:q:s:u:" OPT;do
      case $OPT in
...
if [[ -z "$ATROPOS_SEGMENTATION_MRF" ]];then
...
echo "Started `date`"
echo "Will run Atropos segmentation with K=${ATROPOS_NUM_CLASSES}. Classes labeled in order of mean intensity."
echo "Assuming CSF=${ATROPOS_CSF_CLASS_LABEL}, GM=${ATROPOS_GM_CLASS_LABEL}, WM=${ATROPOS_WM_CLASS_LABEL}"
...
TMP_FILES=( $EXTRACTION_MASK_PRIOR_WARPED $EXTRACTION_WARP $EXTRACTION_INVERSE_WARP $EXTRACTION_TMP $EXTRACTION_INITIAL_AFFINE $EXTRACTION_INITIAL_AFFINE_FIXED $EXTRACTION_LAPLACIAN $EXTRACTION_TEMPLATE_LAPLACIAN $EXTRACTION_WM $EXTRACTION_GM $EXTRACTION_CSF $EXTRACTION_SEGMENTATION $EXTRACTION_INITIAL_AFFINE_MOVING $EXTRACTION_INITIAL_AFFINE $EXTRACTION_BRAIN )

if [[ ! -f ${EXTRACTION_MASK} || ! -f ${EXTRACTION_WM} ]];then
	...
	DENOISED_IMAGE=${OUTPUT_PREFIX}Denoised${i}.${OUTPUT_SUFFIX}

        TMP_FILES=( ${TMP_FILES[@]} $N4_TRUNCATED_IMAGE $DENOISED_IMAGE)
        N4_CORRECTED_IMAGES=( ${N4_CORRECTED_IMAGES[@]} ${N4_CORRECTED_IMAGE} )

        if [[ ! -f ${N4_CORRECTED_IMAGE} ]];then

	    if [[ ${DENOISE_ANATOMICAL_IMAGES} -ne 0 ]];then
                logCmd DenoiseImage -i ${ANATOMICAL_IMAGES[$i]} -o ${DENOISED_IMAGE}
            fi

            logCmd ImageMath ${DIMENSION} ${N4_TRUNCATED_IMAGE} TruncateImageIntensity ${DENOISED_IMAGE} 0.01 0.999 256

            exe_n4_correction="N4BiasFieldCorrection -i ${N4_TRUNCATED_IMAGE} -s ${N4_SHRINK_FACTOR} -c ${N4_CONVERGENCE} -b ${N4_BSPLINE_PARAMS} -o ${N4_CORRECTED_IMAGE}"
    ...
    if [[ ! -f ${N4_CORRECTED_IMAGES[0]} ]];then
    ...
    echo "--------------------------------------------------------------------------------"
    echo "N4 correction done (pre brain extraction) in:  $(( time_elapsed_n4_correction / 3600 ))h $(( time_elapsed_n4_correction %3600 / 60 ))m $(( time_elapsed_n4_correction % 60 ))s"
    echo "--------------------------------------------------------------------------------"
    echo

    if [[ ! -f ${EXTRACTION_INVERSE_WARP} ]];then
        if [[ ! -f ${N4_CORRECTED_IMAGES[0]} ]];then
...
#             exe_initial_align="antsAffineInitializer ${DIMENSION} ${EXTRACTION_INITIAL_AFFINE_FIXED} ${EXTRACTION_INITIAL_AFFINE_MOVING} ${EXTRACTION_INITIAL_AFFINE} 15 0.1 0 10"
          exe_initial_align="antsAI -d ${DIMENSION}"
          ...
          exe_initial_align="${exe_initial_align} -t Affine[0.1] -s [20,0.12]"
          exe_initial_align="${exe_initial_align} -g [40,0x40x40] -p 0 -c 10"
...
#                 exe_initial_align="${exe_initial_align} ${EXTRACTION_REGISTRATION_MASK}"
          ...
          basecall="antsRegistration -d ${DIMENSION} -u 0 -w [0.025,0.975] -o ${EXTRACTION_WARP_OUTPUT_PREFIX} -r ${EXTRACTION_INITIAL_AFFINE} --float ${USE_FLOAT_PRECISION}"
          if [[ -f ${EXTRACTION_REGISTRATION_MASK} ]];then
              basecall="${basecall} -x [${EXTRACTION_REGISTRATION_MASK}]"
          fi
          stage1="-m MI[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},${ANTS_LINEAR_METRIC_PARAMS}] -c ${ANTS_LINEAR_CONVERGENCE} -t Rigid[0.1] -f 8x4x2x1 -s 3x2x1x0vox"
          stage2="-m MI[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},${ANTS_LINEAR_METRIC_PARAMS}] -c ${ANTS_LINEAR_CONVERGENCE} -t Affine[0.1] -f 8x4x2x1 -s 3x2x1x0vox"
          stage3="-m CC[${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},1,4] -c [100x70x50x20,1e-6,10] -t ${ANTS_TRANSFORMATION} -f 8x4x2x1 -s 3x2x1x0vox"
        ...
        if [[ ! -f ${EXTRACTION_GENERIC_AFFINE} ]];then
      ...
      if [[ ! -f ${EXTRACTION_SEGMENTATION} ]];then
      ## Step 2 ##

        exe_brain_extraction_2="antsApplyTransforms -d ${DIMENSION} -i ${EXTRACTION_PRIOR} -o ${EXTRACTION_MASK_PRIOR_WARPED} -r ${ANATOMICAL_IMAGES[0]} -n Gaussian -t [${EXTRACTION_GENERIC_AFFINE},1] -t ${EXTRACTION_INVERSE_WARP} --float ${USE_FLOAT_PRECISION}"
       ...
       # Pad image here to avoid errors from dilating into the edge of the image
        ...
        # De-pad
        for img in ${EXTRACTION_SEGMENTATION} ${EXTRACTION_MASK} ${EXTRACTION_WM} ${EXTRACTION_GM} ${EXTRACTION_CSF} ${EXTRACTION_MASK_PRIOR_WARPED};do
    ...
    echo "--------------------------------------------------------------------------------"
    echo " Brain extraction done in:  $(( time_elapsed_brain_extraction / 3600 ))h $(( time_elapsed_brain_extraction %3600 / 60 ))m $(( time_elapsed_brain_extraction % 60 ))s"
    echo "--------------------------------------------------------------------------------"
    echo
  fi
...
if [[ $KEEP_TMP_IMAGES -eq 0 ]];then
    for f in ${TMP_FILES[@]};do
        if [[ -e $f ]];then
            logCmd rm $f
          else echo "WARNING: expected temp file doesn't exist: $f"
        fi
    done
fi
