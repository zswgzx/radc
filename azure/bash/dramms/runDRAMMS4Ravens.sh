...
# author: Shengwei
# purpose: run Azure Batch in CLI for making RAVENS from T1 image and tissue segmentation maps 
# prerequisites: DRAMMS, Azure CLI (check version by "az -v") installed on Ubuntu 16.04 LTS
# expected run time: ~ 45 mins per scan
# notes: 
# 1. choose a compute optimized VM instead of memory/storage opt. one (e.g. standard_a2m_v2)
...
jobId='ravens'
inputContainer='dramms-inputs-sz'
templateFile='IIT-Aging.nii.gz'
outputContainer='dramms-outputs-sz'
...
if [ `echo "$dedicatedNodes * $vmVCPUs + $coresInUse" | bc ` -gt $ACCNT_CORE_QUOTA ] ;then
	dedicatedNodes=`echo "($ACCNT_CORE_QUOTA/2 - $coresInUse)/$nodeMaxTasks+1" | bc`
	...
	dedicatedNodes=`echo "$dedicatedNodes/$nodeMaxTasks+1" | bc`
...
echo -e "\$NodeDeallocationOption=taskcompletion;\$tasks=(\$ActiveTasks.GetSamplePercent(5*TimeInterval_Minute)<70) ? max(0,\$ActiveTasks.GetSample(1)) : max(\$ActiveTasks.GetSample(1),avg(\$ActiveTasks.GetSample(TimeInterval_Minute*5)));\$targetVMs = \$tasks>0 ? \$tasks : max(\$RunningTasks.GetSample(TimeInterval_Minute*5))/$nodeMaxTasks+1;\$TargetDedicated = min($dedicatedNodes, \$targetVMs)" > scaleFormula
...
for subjid in `ls $inputContainer|grep ^1|cut -d- -f1`; do
	echo -e "{
  \"id\": \"task$count\",
  \"commandLine\": \"/bin/bash -c '. /usr/local/software/addpaths;ln -s /mnt/batch/tasks/startup/wd/$templateFile;dramms -S t1-dramms.nii.gz -T $templateFile -O template-to-$subjid.nii.gz -D template-to-$subjid-def.nii.gz -R $subjid-ravens -L tissue-seg.nii.gz -l 1,2,3;cp ../stdout.txt $subjid.log'\",
  \"resourceFiles\": [
    {
      \"blobSource\": \"https://$storageName.blob.core.windows.net/$inputContainer/$subjid/t1-dramms.nii.gz?$sasIn\",
      \"filePath\": \"t1-dramms.nii.gz\"
    },
    {
      \"blobSource\": \"https://$storageName.blob.core.windows.net/$inputContainer/$subjid/tissue-seg.nii.gz?$sasIn\",
      \"filePath\": \"tissue-seg.nii.gz\"
    }
  ],
  \"userIdentity\": {
    \"autoUser\": {
