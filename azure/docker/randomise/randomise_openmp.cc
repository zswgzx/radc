...
public:
  ...
  vector<ColumnVector> allPermutations;
...
void calculatePermutationStatistics(ranopts& opts, const volume<float>& mask, Matrix& datam, const Matrix& tc, Matrix& dm,int tstatnum, vector<int>& dof, Permuter& permuter, VoxelwiseDesign& voxelwiseDesign)
{
  ...
  // Create unique permutations outside of for loop. The next permutation depends on previous permutations. All permutations stored in a vector called allPermutations
  ColumnVector permvec;
  cout << "Setting up permutations (Not running permutations)" << endl;
  for(unsigned long perm=1; perm<=nPerms; perm++) {
    permvec = permuter.nextPermutation(perm,0, isNormalising || opts.outputTextPerm.value());
    permuter.allPermutations.push_back(permvec);
  }

  cout << "------ Starting computations... " << endl;

  // TFCE depends on first permutation result, so the first permutation is performed before the parallelized loop. Based on testing, TFCE result can change.
//  for(unsigned long perm=1; perm<=1; perm++) {
//    Matrix tstat(1,nVoxels); //tstat becomes cleared when using pragma omp, tstat.Column(voxel) in the voxelwiseDesing will fail without this line

  //  cout << "Starting permutation " << perm << endl;
  //  permvec=permuter.allPermutations[perm-1];
  ...
  //  if( opts.var_sm_sig.value()>0 && tstatnum > 0 ) 
   ...
   //   cerr << "statistic Maximum: " << tstat.Maximum() << endl;
    ...
    //store method has several instances of += operations, probably could use atomic or reduced to increase speed. Not sure how to do. pragma omp critical is tested to generate the same results without openmp.
...
//    Matrix tstat(1,nVoxels); //tstat becomes cleared when using pragma omp, tstat.Column(voxel) in the voxelwiseDesing will fail without this line
    ...
    if (voxelwiseDesign.isSet)
//      #pragma omp parallel default(none) private(dmperm,tstat,permvec,pe,cope,varcope,sigmaSquared) shared(nPerms,isNormalising,opts,permuter,dm,voxelwiseDesign,datam,cout,tstatnum,rankF,dof,tc,smoothedMask,mask,cerr,voxels,tfce_delta,clusterEnhanced,lowram,previousTFCEStat,clusterThreshold,clusters,massThreshold,clusterMasses,nVoxels)
      #pragma omp parallel default(none) private(dmperm,pe,cope,varcope,sigmaTemp) shared(sigmaSquared,tstat,permvec,nPerms,isNormalising,opts,permuter,dm,voxelwiseDesign,datam,cout,tstatnum,rankF,dof,tc,smoothedMask,mask,cerr,voxels,tfce_delta,clusterEnhanced,lowram,previousTFCEStat,clusterThreshold,clusters,massThreshold,clusterMasses,nVoxels)
      {
      #pragma omp for ordered schedule(dynamic)
      for(int voxel=1;voxel<=datam.Ncols();voxel++) {
	Matrix dmtemp(voxelwiseDesign.adjustDesign(dm,voxel)), sigmaTemp,peTemp,copeTemp,varcopeTemp;
	dmperm=PermutedDesign(dmtemp,permvec,permuter.isFlipping);
        Matrix tstatTemp(1,nVoxels);
	tstat.Column(voxel)=evaluateStatistics(datam.Column(voxel), dmperm, voxelwiseDesign.contrastAtVoxel[voxel], pe, cope, varcope, sigmaTemp, voxelwiseDesign.dofAtVoxel[voxel], rankF, opts.nMultiVariate.value(), (tstatnum < 0) );
//        #pragma omp critical
  //      {
	//tstat.Column(voxel)=tstatTemp.Column(voxel);
        if ( opts.var_sm_sig.value()>0 && tstatnum > 0 ) 
	  sigmaSquared.Column(voxel)=sigmaTemp;
    //    }
      }
