...
% read list of subjects to process
subjs=importfile('subjs');

% generate list of input files (N4 corrected T1w) for batch
for i=1:length(subjs)
    system(['gunzip ',subjs{i},'-n4.nii.gz']);
    matlabbatch{1}.spm.tools.cat.estwrite.data{i,1} = ['/home/sz/cat/',subjs{i},'-n4.nii,1'];
end

% CAT12 parameters

matlabbatch{1}.spm.tools.cat.estwrite.nproc = 14; % change this according to vCPUs of VM!!

matlabbatch{1}.spm.tools.cat.estwrite.opts.tpm = {'/usr/local/spm/spm12v7219/tpm/TPM.nii'};
