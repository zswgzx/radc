...
def find_dicom( base_path ):
    ...
    #         if re.search("nii|nifti|nifti|par|P\d{5,6}.zip|phantom|acr", filename, re.IGNORECASE) is None:
    #             temp_path = os.path.join( root, filename)
    #             if(zipfile.is_zipfile( temp_path) ):
    #                 if( check_dicom_zip( ZipFile(temp_path) ) ):
    #                    return temp_path
    #
    #             elif(tarfile.is_tarfile( temp_path) ):
    #                 if( check_dicom_tar(tarfile.open(temp_path, 'r|gz'))):
    #                     return temp_path
